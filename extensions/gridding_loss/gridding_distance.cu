...
std::vector<torch::Tensor> gridding_distance_cuda_forward(float min_x,
  ...
  gridding_dist_kernel<<<batch_size, get_n_threads(n_pts), 0, stream>>>(n_grid_vertices, n_pts, min_x, min_y, min_z, len_y, len_z,
    ptcloud.data_ptr<float>(), grid_weights.data_ptr<float>(),
    grid_pt_weights.data_ptr<float>(), grid_pt_indexes.data_ptr<int>());
...
std::vector<torch::Tensor> gridding_distance_cuda_backward(torch::Tensor grid_pt_weights,
                                                           torch::Tensor grid_pt_indexes,
                                                           torch::Tensor grad_grid) {
  int batch_size      = grad_grid.size(0);
  int n_pts           = grad_grid.size(1);
  int len_x           = grid_pt_weights.size(1);
  int len_y           = grid_pt_weights.size(2);
  int len_z           = grid_pt_weights.size(3);
  int n_grid_vertices = len_x * len_y * len_z;

  torch::Tensor grad_ptcloud =
    torch::zeros({batch_size, n_pts, 3}, torch::CUDA(torch::kFloat));

  gridding_dist_grad_kernel<<<batch_size, get_n_threads(n_pts), 0, 0>>>(n_grid_vertices,
                                                                         n_pts,
                                                                         grid_pt_weights.data_ptr<float>(),
                                                                         grid_pt_indexes.data_ptr<int>(),
