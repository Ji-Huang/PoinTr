...
#include <cmath>          // for std::log and std::cbrt
#include <algorithm>      // for std::max and std::min
#include <cstdio>         // for printf
#include <cstdlib>        // for std::exit
#include <torch/extension.h> // for PyTorch extension API

#define CUDA_NUM_THREADS 512
#define EPS 1e-6

// Compute the number of threads needed in GPU
  ...
  float *__restrict__ grad_grid) {
    ...
    for (int j = index; j < n_pts; j += stride) {
        ...
        int gvtx_indexes[8] = {
            ...
            j};
        float weights[8] = {
            grid[gvtx_indexes[0]], grid[gvtx_indexes[1]],
        ...
        for (int i = 0; i < 8; ++i) {
            atomicAdd(&(grad_grid[gvtx_indexes[i]]),
                grad_ptcloud[j * 3 + 0] * (x_offset - ((i & 1) ? x_offset : x_offset - 1)) / weights_sum +
                grad_ptcloud[j * 3 + 1] * (y_offset - ((i & 2) ? y_offset : y_offset - 1)) / weights_sum +
                grad_ptcloud[j * 3 + 2] * (z_offset - ((i & 4) ? z_offset : z_offset - 1)) / weights_sum);
        }
    }
}
...
torch::Tensor gridding_reverse_cuda_backward(torch::Tensor ptcloud,
    ...
    gridding_reverse_grad_kernel<<<batch_size, get_n_threads(n_pts), 0, stream>>>(
        scale, n_pts, ptcloud.data_ptr<float>(), grid.data_ptr<float>(), grad_ptcloud.data_ptr<float>(), grad_grid.data_ptr<float>());
