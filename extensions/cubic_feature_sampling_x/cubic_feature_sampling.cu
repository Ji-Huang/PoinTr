...
__global__ void cubic_feature_sampling_kernel(
  ...
  for (int i = index; i < n_pts; i += stride) {
    float pt_x = static_cast<float>(ptcloud[i * 3 + 0]);
    float pt_y = static_cast<float>(ptcloud[i * 3 + 1]);
    float pt_z = static_cast<float>(ptcloud[i * 3 + 2]);

    int lower_x = floorf(pt_x);
    int upper_x = ceilf(pt_x);
    if (lower_x == upper_x) upper_x += 1;
    int lower_y = floorf(pt_y);
    int upper_y = ceilf(pt_y);
    if (lower_y == upper_y) upper_y += 1;
    int lower_z = floorf(pt_z);
    int upper_z = ceilf(pt_z);
    if (lower_z == upper_z) upper_z += 1;
      ...
      for (int k = lower_y - ns; k <= upper_y + ns; ++k) {
        for (int m = lower_z - ns; m <= upper_z + ns; ++m) {
          if (j < 0 || j >= scale || k < 0 || k >= scale || m < 0 || m >= scale) {
            ...
            grid_pt_indexes[i * n_vertices + vertex_idx++] = compute_index(j, k, m, scale);
          }
        }
      }
    }
  ...
  for (int i = index; i < n_pts; i += stride) {
    for (int j = 0; j < n_vertices; ++j) {
      ...
      for (int k = 0; k < n_cubic_channels; ++k) {
        ...
        float grad_val = static_cast<float>(grad_point_features[grad_idx]);

        int index = k * cub_scale + vertex_idx;

        // Handling Half precision with atomicAdd
        if (std::is_same<scalar_t, at::Half>::value) {
          // Cast to float and then do atomicAdd
          atomicAdd(reinterpret_cast<float *>(&grad_cubic_features[index]), grad_val);
        } else {
          // Regular atomicAdd for float types
          atomicAdd(&grad_cubic_features[index], static_cast<scalar_t>(grad_val));
        }
      }
    }
  }
...
std::vector<torch::Tensor> cubic_feature_sampling_cuda_forward(
  ...
  auto options = ptcloud.options();
  torch::Tensor point_features =
    torch::zeros({batch_size, n_pts, n_vertices, n_cubic_channels}, options);
  torch::Tensor grid_pt_indexes =
    torch::zeros({batch_size, n_pts, n_vertices},
                 torch::TensorOptions().dtype(torch::kInt).device(ptcloud.device()));

  AT_DISPATCH_FLOATING_TYPES_AND_HALF(ptcloud.scalar_type(), "cubic_feature_sampling_forward", [&] {
    cubic_feature_sampling_kernel<scalar_t><<<batch_size, get_n_threads(n_pts), 0, stream>>>( 
      scale, neighborhood_size, n_vertices, n_pts, n_cubic_channels, 
...
std::vector<torch::Tensor> cubic_feature_sampling_cuda_backward(
  ...
  int n_vertices       = grid_pt_indexes.size(2);
  int n_cubic_channels = grad_point_features.size(3);

  auto options = grad_point_features.options();
  torch::Tensor grad_ptcloud =
    torch::zeros({batch_size, n_pts, 3}, options);
  torch::Tensor grad_cubic_features =
    torch::zeros({batch_size, n_cubic_channels, scale, scale, scale}, options);

  AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad_point_features.scalar_type(), "cubic_feature_sampling_backward", [&] {
    cubic_feature_sampling_grad_kernel<scalar_t><<<batch_size, get_n_threads(n_pts), 0, stream>>>( 
      scale, neighborhood_size, n_vertices, n_pts, n_cubic_channels,
